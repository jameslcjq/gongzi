...
**R-13 版本标识混乱**：package.json `1.0.12` 与 artifactName 手写 `1.12` 双轨；4 个变体安装包重打后同名同版本——6 月已实际造成"装了旧包以为是新包"。版本号单一来源 + artifact 带构建时间。

**R-14 dev-userscripts 生成漂移**：每次重新生成 03/04 会出现与提交版不一致的 diff（源 TS 演进了而产物未跟随提交）。对齐一次后整体提交，或生成器支持指定脚本。
...
| 持续 | 巨型组件只拆不增、错误约定统一、版本治理 | R-04/11/13 | 随迭代 |
