...
async function readBudgetWorkbookImports(
  filePath: string,
...
function findSheetForWorksheet(
  workbook: XLSX.WorkBook,
  worksheet: WorksheetMeta,
...
function shouldUseMultiRowConfig(
  sheet: XLSX.WorkSheet,
...
function validateHeaderSignature(headers: string[], worksheet: WorksheetMeta): void {
  ...
  for (let index = 0; index < Math.min(actual.length, expected.length); index += 1) {
    ...
    issues.push(`? ${index + 1} ??${actualHeader || '(?)'} -> ?? ${want}`)
    if (issues.length >= 6) break
  }
  if (extra.length > 0 || missing.length > 0 || issues.length > 0) {
    const parts = [`?${worksheet.name}?????? Excel ??????????`]
    if (extra.length > 0) parts.push(`?????${extra.slice(0, 12).join('?')}`)
    if (missing.length > 0) parts.push(`?????${missing.slice(0, 12).join('?')}`)
...
function parseWorkHistoryLine(raw: string): Record<string, string> {
